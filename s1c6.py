...
def find_xor_key(data,top=1):
    ...
    for x in hams[:top]:
        ...
        key=''.join(make_key)
        #print(transposed_joined)
        #print("Key:",key)
        #print(binascii.unhexlify(enc_rep(b64decode(raw_data),key)))
        answer+=[(float(x[0]),x[1]/2,key)] #Hamming score,keylen,key
    return answer
...
if __name__ == '__main__':
    ...
    for i in find_xor_key(data,3):
        print(i)
        #print(binascii.unhexlify(enc_rep(b64decode(raw_data),i[2])))
